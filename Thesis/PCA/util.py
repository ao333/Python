...
def hists(data, bins=10, size='auto', c='steelblue', ctop=False, fs=12):
    if size == 'auto':
        ...
        rows = int(np.ceil(len(list(data)) / cols))  # optimal number of rows for square matrix of histograms
    else:
        cols = size[0]
        rows = size[1]
...
def correlation(data, col='return', exclude=None, values=False, show=True, save=False, ret_col=False):
