...
def generate_indicators(sd = dt.datetime(2008,1,1), ed = dt.datetime(2009,12,31), \
    syms = ['AAPL'], allocs=[1.0], sv=100000, rfr=0.0, sf=252.0, gen_plot=True):
...
if __name__ == "__main__":
    ...
    stock_prices, bbp, sma_price, momen, d_rets, Y = generate_indicators(sd = start_date, ed = end_date,\
        syms = symbols, allocs = allocations,sv = start_val, gen_plot = 'train')
    ...
    test_prices, test_bbp, test_sma_price, test_momen, test_d_rets, tY = generate_indicators(sd = test_start_date, ed = test_end_date,\
        syms = symbols, allocs = allocations,sv = start_val, gen_plot = 'test')
    ...
    plt.title('High Frequency Strategy', fontsize=16)
    ...
    plt.clf()
    plt.plot(qlongs[:,0],qlongs[:,2], 'go', label='Wins', ms=8)
    plt.plot(qshorts[:,0],qshorts[:,2], 'rh', label = 'Losses', ms=8)
    plt.plot(qdo_nothing[:,0],qdo_nothing[:,2], 'k*', label = 'Breakeven', ms=8)
    plt.title('Trade And Hold Strategy', fontsize=16)
    ...
    plt.clf()
    plt.plot(rlongs[:,0],rlongs[:,2], 'go', label = 'Wins', ms=8)
    plt.plot(rshorts[:,0],rshorts[:,2], 'rh', label = 'Losses', ms=8)
    plt.plot(rdo_nothing[:,0],rdo_nothing[:,2], 'k*', label = 'Breakeven', ms=8)
    plt.title('High Frequency Long-Short Strategy', fontsize=16)
    plt.ylabel('Sharpe Ratio', fontsize=16)
    plt.xlabel('Signal Strength', fontsize=16)
    plt.legend(loc="upper left")
    plt.show()
