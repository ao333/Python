...
class RTLearner(object):
    ...
    def addEvidence(self, trainX, trainY):
        ...
        def build_tree(trainX, trainY):  # cannot be called from outside this class
            """
            @summary: Builds the Tree recursively by randomly choosing feature to split on and the splitting value
            ...
            if trainX.shape[0] <= self.leaf_size:  # if no. of train examples < leaf_size, aggregate into leaf
                ...
                while i < 5:
                    ...
                    # randomly choose split value of feature
                    splitVal = np.mean(np.random.choice(trainX[:, feat], 2, replace=False))
                    indLeft = trainX[:, feat] <= splitVal  # logical array for indexing
                    indRight = ~indLeft  # complement of indLeft
                    if (indLeft[:]==1).all() != True:
                        break  # break out of while loop if selected feature vector is divided into two groups

                if i < 5 or not(indLeft[:] == 1).all():  # if labels divided into 2 groups
                    ...
                    if self.verbose:
                        print('Node appears to be terminal with feature = {}, splitVal = {}:'
                              '5 attempts made yet cannot partition node, retrying...!'.format(feat, splitVal))
