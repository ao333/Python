...
import numpy as np
import pandas as pd
from RT import RTLearner
import matplotlib.pyplot as plt
from indicators import defineData, createOrder
from marketsim import testcode_marketsim
from rule_based import getSmaIndicator, getMomIndicator, getCrossIndicator, getBbIndicator, plotVline, tradingStrategy


def optimizeY(prices):
    """
    @Summary: Optimize Ybuy/Ysell parameters by plotting averaged error & 
              cumulative return for training/cross-val datasets vs. Ybuy (assuming Ybuy = Ysell)
    @param prices: Series, contains adj. close price with date indices
    """
    numParam = 13
    tryParam = np.logspace(-4, -1, num=numParam)  # values of Ybuy = Ysell to iterate over
    fname = 'ML_Strat'  # name of orders file
    cumRetTrain = np.zeros(numParam)
    cumRetCrossVal = np.zeros(numParam)
    accTrain = np.zeros(numParam)
    accCrossVal = np.zeros(numParam)
    learner = RTLearner(leaf_size=5, verbose=False)  # create learner
    reps = 5  # number of RTclassLearners whose predictions to average over
    start = time.time()
    for i in range(numParam):  # X is a ndarray, Y is a 1darray
        ...
        for j in range(reps):
            learner.addEvidence(trainX, trainY)  # train on trainX, trainY
            ...
            cumRetTrain[i] += cumReturn  # cross-validation sample querying
    ...
    stop = time.time()
    print 'Time to calculate cumulative return & error vs the selected parameters: {} s'.format(stop - start)
...
def getXY(prices, Ybuy=0.01, Ysell=0.01, holdTime=21):
    ...
    the present price on the basis of which to classify the present price
    @param holdTime: int, used to classify present prices based on return after holdTime number of days
...
def optimizeParam(X, Y, dates):
    """
    @Summary: Optimize leaf size of RTclassLearner or number of bags of BagClassLearner by plotting averaged error
    and cumulative return for training & cross-val datasets vs. leaf size / number of bags
    @param X: ndarray, of technical indicators. Examples in rows, features in columns
    @param Y: 1darray, of the labels -1 (sell), 0 (hold), and 1 (buy) based on future returns after a holding time period
    """
    numParam = 1
    tryParam = np.linspace(75, 75, num = numParam)  # values of leaf size / number of bags to iterate over
    fname = 'ML_Strat'  # name of orders file
    cumRetTrain = np.zeros(numParam)
    cumRetCrossVal = np.zeros(numParam)
    accTrain = np.zeros(len(tryParam))
    accCrossVal = np.zeros(len(tryParam))
    reps = 10  # number of configurations of RTclassLearners or BagClassLearners whose predictions to average over
    split = int(0.6 * len(Y))  # divide into train and cross-val sets
    trainX = X[:split - 1, :]
    trainY = Y[:split - 1]
    trainDates = dates[:split - 1]
    ...
    for i in range(numParam):  # initialize either RTclassLearner/BagClassLearner
        learner = RTLearner(leaf_size=tryParam[i], verbose = False)  # create learner
        print tryParam[i]  # print parameter values during computation
        ...
        for j in range(reps):
            ...
            cumRetCrossVal[i] += cumReturn
    stop = time.time()
    print 'Time to calculate error vs the selected parameter: {} s'.format(round( stop - start, 2))
...
if __name__ == '__main__':
    ...
    prices = defineData(startDate='01-01-2008', stopDate='31-12-2009', symList=['AAPL'])  # in-sample dates
    optimizeY(prices)  # UNCOMMENT to optimize Ybuy/Ysell
    Ybuy = 0.01
    Ysell = 0.01
    holdTime = 21
    X, Y, dates = getXY(prices, Ybuy, Ysell, holdTime)  # X is an ndarray, Y is a 1darray
    optimizeParam(X, Y, dates)  # UNCOMMENT to optimize leaf size of RTclassLearner or number of bags of BagClassLearner

    learner = RTLearner(leaf_size=75, verbose=False)  # create learner
    ...
    plt.title('in-sample mc3_p1.data and labels before training')
    lg = plt.legend(loc='best')
    lg.draw_frame(True)
    ...
    testPrices = defineData('01-01-2010', '31-12-2011')  # out of sample testing after training on training set
    testX, testY, testDates = getXY(testPrices, Ybuy, Ysell, holdTime)  # testX is an ndarray, testY is a 1darray
    cumRetTest = 0.0
    accTest = 0.0
    reps = 1  # number of configuratiokns of RTclassLearners whose predictions to average over
    ...
    for j in range(reps):
        ...
        signal = pd.Series(pred, index=testDates)  # Generate order from out of sample predictions
        order = tradingStrategy(signal)
        createOrder(order, 'ML_Strat')
        cumReturn, portVals = testcode_marketsim('ML_Strat', verbose=False)
