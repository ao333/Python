"""
Development of a few standard technical indicators including a custom "crossover" indicator. Also contains a routine to
evaluate the maximum possible return with a given stock looking into the future and with a restriction of +/- 200 shares
...
def createOrder(order, fname, base_dir='', symbol='AAPL', unit=200):
    """
    @summary: Takes an order, a filename, and a base directory and writes a csv order file
    @param order: Series, consists of the values -2, -1, 0, 1, 2 denoting holding or buying/selling of 200 or 400 shares
    ...
    for ind, val in order.iteritems():  # iterator over the timestamp indices and values in 'order'
        if val == 1:
...
def bestPossibleStrategy(data):
    """
    @Summmary: Evaluate the maximum possible return with a given stock looking into the future and with a restriction
    of +/- 200 shares per transaction with 0, 200 (represented by 1), -200 (represented by -1) being the only allowed
    positions (hold, buy 200 and sell 200)
    @param data: Series, contains adj. close prices of the stock with date indices
    @returns order: Series, consists of the values -1, 0, 1, denoting selling/buying of 200 shares or just holding
    """
    # calculate today's price relative to tomorrow's price. This is to decide whether to buy/sell today
    nextDayReturn = data.ix[:-1] / data.values[1:] - 1
    # restore the last date/value row which was removed by the previous step
    nextDayReturn = nextDayReturn.append(data[-1:])
    nextDayReturn[-1] = np.nan  # The value of the last row is a NaN since we do not know the next day's price
    ...
    order = dailyOrder.diff(periods=1) / 2
    # pick out only where 1 changes to -1 and vice-versa while eliminating consecutive 1s and -1s.
    # Division by 2 needed since we are constrained to buying and selling only 200 shares at a time.
    order[0] = dailyOrder[0]  # restore the first date/value row removed by the previous differentiation operation
    if order.sum() == -1:  # on the last day, close any open positions
        order[-1] = 1
    elif order.sum() == 1:
        order[-1] = -1
...
def getCross(data, crossWindow = 25):
    """
    @Summary: Trigger buy/sell signals at the minima/maxima of a slowly oscillating time-series where it equals and
    crosses its moving average but with an opposite sign of slope
    ...
    slow oscillations. On days where the slopes of the time-series and its sma have the same sign, the value is set to 0.
    On days where the slopes have opposite sign but a crossover does not occur, values between -1 and 1 are returned
    """
    value = 1 / (1 + data / getSma(data, crossWindow))  # ranges from 0 to 1, usually around 0.5
    # set sign = 1 (buy) for prices going up in time and sma going down.
    # set sign = -1 (sell) for the opposite situation.
    sign = (1 * ((data.diff(periods=crossWindow) > 0) & (getSma(data, crossWindow).diff(periods=crossWindow) < 0))
            -1 * ((data.diff(periods=crossWindow) < 0) & (getSma(data, crossWindow).diff(periods=crossWindow) > 0)))
...
def getMom(data, momWindow=5):
    """
    @Summary: Calculate momentum indicator = ratio of price with respect to price momWindow number of days back - 1
    @param data: Series, contains prices with date indices
    @param momWindow: int, number of days to look back
    @returns Series of the same size as input mc3_p1.data
    """
    diff = data.diff(periods=momWindow)  # difference of prices wrt price momWindow number of days back
...
def getBb(data, bbWindow=20):
    ...
    @param bbWindow: int, number of days to look back to calculate the moving average and moving standard deviation
    @returns Series of the same size as input mc3_p1.data
    """
