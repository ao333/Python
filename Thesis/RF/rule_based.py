...
from indicators import defineData, getSma, getMom, getBb, getCross, standardize, createOrder
...
def tradingStrategy(signal, holdTime=21):
    ...
    @param signal: Series, consists of the values -1, 0, 1 denoting sell, holdor buy
    @param holdTime: int, holding period after a transaction
    @returns order: Series, consists of the values -2, -1, 0, 1, 2 denoting  selling/buying of 200 or 400 shares
    ...
    # order can take values of  0, 1, -1 corresponding to 0, +/-200 shares
        if (currOrder < 1) and (signal[day] == 1):  # current order status is not long and signal to buy is given
            order[day] = 1  # buy 200
            currOrder += order[day]
            day += holdTime  # after buying wait for hold period
        elif (currOrder > -1) and (signal[day] == -1):  # current order status is not short and signal to sell is given
        ...
        if day >= numDays:  # if we reach the end of the trading period, redeem all outstanding positions
            if currOrder == 1:
                order[-1] = -1
            elif currOrder == -1:
                order[-1] = 1
...
if __name__ == '__main__':
    ...
    smaSignal = 1 * (smaIndicator < -smaThreshold) + -1 * (smaIndicator > smaThreshold)
    ...
    momSignal = -1 * (momIndicator < -momThreshold) + 1 * (momIndicator > momThreshold)
    ...
    plt.subplot(313)
    plt.scatter(momIndicator[signal == 0], crossIndicator[signal == 0], color='k', label='hold')
    plt.scatter(momIndicator[signal == 1], crossIndicator[signal == 1], color='g', label='buy')
    plt.scatter(momIndicator[signal == -1], crossIndicator[signal == -1], color='r', label='sell')
    lg = plt.legend(loc='best')
    lg.draw_frame(True)
