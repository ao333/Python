...
import QStrategy as sl
from matplotlib import style
from util import get_data
style.use('ggplot')


def test_code(sym, c0, insample=False, verb=True):
    # instantiate the strategy learner
    learner = sl.StrategyLearner(verbose=verb)
    # set parameters for training
    sd = dt.datetime(2008, 1, 1)
    ed = dt.datetime(2009, 12, 31)  # just a few days for "shake out"
    # train the learner
    t0 = time.clock()
    learner.addEvidence(symbol=sym, sd=sd, ed=ed, sv=c0)
    t1 = time.clock()
    ...
    # get some mc3_p1.data for reference
    syms = [sym]
    ...
    # test the learner
    t2 = time.clock()
    df_trades = learner.testPolicy(symbol=sym, sd=sd, ed=ed, sv=c0)
    t3 = time.clock()
