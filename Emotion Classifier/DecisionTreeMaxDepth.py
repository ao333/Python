...
def Decision_Tree_Learning(examples, attributes, binary_targets, max_depth = None, current_depth = 1):
    leaf = TreeNode()
    ...
    if max_depth == None or (int(max_depth) and current_depth<max_depth):
        ...
        if len(examples_p) == 0 or len(examples_n) == 0:
            ...
            p_subtree = Decision_Tree_Learning(examples_p, newattribute_p, binary_p, max_depth, current_depth+1)
            p_subtree.sign = 1
            leaf.add_child(p_subtree)
            n_subtree = Decision_Tree_Learning(examples_n,newattribute_n, binary_n, max_depth, current_depth+1)
            n_subtree.sign = 0
