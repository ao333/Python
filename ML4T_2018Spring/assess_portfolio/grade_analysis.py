...
# Check sharp ratio, avg daily ret, and cum_ret (BPH)
...
def test_analysis(inputs, outputs, description, grader):
    ...
    try:
        if not main_code in globals():
            import importlib
            mod = importlib.import_module(main_code) # * Import module
        ...
        def timeoutwrapper_analysis():
            student_rv = analysis.assess_portfolio(sd=start_date, ed=end_date, syms=symbols, allocs=allocs, sv=start_val,
                                                   rfr=risk_free_rate, sf=252.0, gen_plot=False)
            return student_rv
        ...
        for i in xrange(len(tb_list)):
            row = tb_list[i]
            tb_list[i] = (os.path.basename(row[0]), row[1], row[2], row[3])  # show filename instead of absolute path
